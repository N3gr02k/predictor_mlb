...
def get_games_for_date(date):
    ...
    try:
        ...
        if not schedule_data:
            return []
        return schedule_data[0].get('games', [])
...
def process_game_data(game):
    ...
    # Solo procesamos juegos que han finalizado
    if game['status']['abstractGameState'] != 'Final':
        print(f"Omitiendo partido no finalizado: {away_team['name']} @ {home_team['name']}")
        return None
    ...
    try:
        ...
        print(f"Procesando: {away_team['name']} @ {home_team['name']} (Lanzadores: {away_pitcher_name} vs {home_pitcher_name})")

        # Recopilamos todas las características usando nuestro motor de enriquecimiento
        home_pitcher_stats = de.get_recent_pitcher_stats(home_starter_id, game_season, game_date - timedelta(days=30), game_date)
        ...
        away_pitcher_stats = de.get_recent_pitcher_stats(away_starter_id, game_season, game_date - timedelta(days=30), game_date)
...
if __name__ == "__main__":
    # Definimos el rango de fechas para construir nuestro dataset
    ...
    if all_game_features:
        ...
        output_filename = f"mlb_dataset_{start_date.strftime('%Y_%m')}.csv"
